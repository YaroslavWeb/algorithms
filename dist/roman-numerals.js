"use strict";
const assertProxy = (value, expected) => {
    try {
        if (expected.every((item, i) => item === value[i]) &&
...
const assertRoman = (value, expected, arg) => {
    if (value === expected) {
...
const parseRoman = (roman) => {
    return roman.split('').reduceRight(({ sum, last }, char) => {
...
Object.setPrototypeOf(Number.prototype, new Proxy(Object.create(Number.prototype), {
    get(_, prop, rec) {
